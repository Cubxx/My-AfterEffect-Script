(function (that) {
    ...
    var config = {
        ...
        version: 'v1.0',
        auto_updata: true,
        updata_url: 'https://api.github.com/repos/Cubxx/My-AfterEffect-Script/releases/latest',
    ...
    var p = {
        layers_path: function () { return [config.root_path, config.name, 'layers'].join('\\'); },
        response_path: function () { return [config.root_path, config.name, 'response.tmp'].join('\\'); },
        config_path: function () { return [config.root_path, config.name, 'config.json'].join('\\'); },
        init: function () {
            var file = f.repair_path(p.config_path());
            if (file.exists) {
                config = JSON.parse(f.read(p.config_path()));
            }
            else {
                f.write(p.config_path(), JSON.stringify(config));
            }
            config.auto_updata && a.update();
        }
    };
    var a = {
        "import": function () {
            var datas = f.multi_open(p.layers_path());
            if (!datas)
                return;
        ...
        "export": function () {
            ...
            f.multi_save(p.layers_path(), datas);
        },
        UI: function () {
        ...
        update: function () {
            if ($.os.indexOf('Windows') === -1) {
                abort('更新失败: 只支持Windows系统');
            }
            var filePath = p.response_path();
            system.callSystem('cmd /c powershell -Command "Invoke-WebRequest -Uri ' + config.updata_url + ' -OutFile ' + filePath + '"');
            var _a = JSON.parse(f.read(filePath)), tag_name = _a.tag_name, body = _a.body, zipball_url = _a.zipball_url;
            if (tag_name === config.version) {
                alert('已是最新版');
            }
            else if (confirm('是否下载最新版: ' + [tag_name, body].join('\n'))) {
                system.callSystem('cmd /c start ' + zipball_url);
            }
        }
    };
    p.init();
