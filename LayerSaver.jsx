(function (that) {
    var a = cubx.a, b = cubx.b, u = cubx.u, f = cubx.f;
    ...
    var s = {
        ...
        export_layer: function (layer) {
            ...
            switch (type) {
                ...
                case 'Camera': ;
                case 'Light': ;
                default: a.abort('图层类型错误: ' + layer.matchName);
            }
        ...
        "import": function () {
            ...
            datas.each(function (value) {
                s.import_layer(JSON.parse(value));
            });
        },
        "export": function () {
        ...
        UI: function () {
            ...
            btn(win, File.decode(icons["import"]), s["import"]);
            btn(win, File.decode(icons["export"]), s["export"]);
            btn(win, File.decode(icons.setting)).onClick = function () {
                var win = u.dialog(void 0, '设置面板').assign({
                    margins: 10
                });
                var panel = u.panel(win, '').assign({
                    orientation: 'column',
                    alignChildren: 'left'
                });
                u.button(u.statictext(panel, '导出路径'), '选择文件夹').assign({
                    onClick: function () {
                        var folder = new Folder(LAYER_PATH).selectDlg();
                        if (folder) {
                            LAYER_PATH = folder.fsName;
                        }
                    }
                });
                u.checkbox(panel, '自动检查更新').assign({
                    value: true,
                    onClick: function () {
                        alert(this.value)
                    }
