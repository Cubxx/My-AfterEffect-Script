    ...
    const _cubx = {
        ...
        a: {
            ...
            emptyArray(array: _PropertyClasses[]) {
                ...
                const group_info_arr = array.map((e, i) => {
        ...
        b: {
            ...
            //add layer
            add_layer: ((type = 'Shape') => {
                const layer = b.get_active_comp().layers['add' + type]?.();
                if (layer === void 0) {
                    a.abort('添加图层错误\t' + type);
                }
                return layer;
            ...
            add_layers_from_selected_groups() {
                ...
                const groups = properties.filter((e): e is PropertyGroup => e instanceof PropertyGroup && !(e instanceof MaskPropertyGroup));
                a.checkLength(groups, '请选择属性组(除蒙版以外)');
                const beDels = groups.map(e => (b.add_layer_from_group(e), e));
                a.emptyArray(beDels);
            },
            // unpack
            ...
            unpack_layer(layer: ShapeLayer) {
                layer.selected = true;
                const contents = layer.property('ADBE Root Vectors Group').is(PropertyGroup);
                const group_array = contents.map(e => {
        ...
        u: {
            ...
            palette: ((that: any, text = '') => {
                return (that instanceof Panel) ? that : new Window('palette', void 0, void 0, { resizeable: true }).assign({
                    orientation: 'column',
                    alignChildren: 'left',
                    margins: 0,
                    spacing: 0,
            ...
            dialog: ((that: any, text = '') => {
                return (that instanceof Panel) ? that : new Window('dialog', void 0, void 0, { resizeable: true }).assign({
                    orientation: 'column',
                    alignChildren: 'left',
                    margins: 0,
                    spacing: 0,
        ...
        f: {
            ...
            open(config?: { path?: string, prompt?: string, filter?: string, multi?: boolean }): Record<string, string> | void {
                const { path, prompt, filter, multi } = config;
                const files = path !== void 0
...
(function () {
    /**
    * 构建遍历函数
    * @invoke (数组, 获取长度函数) (获取元素函数) (回调函数, 遍历顺序)
    */
    function map_factory<T>(data: T, getLength = (arr: T) => arr['length']) {
        ...
        return function <U>(getValue: (array: T, index: number) => U) {
            return function <R>(fn: (value: U, index: number, array: T) => R, order?: boolean): R[] {
                const array = [];
                for (let i = len - 1; i >= 0; i--) {
                    // order: true 正序; false 倒序
                    const index = order ? len - 1 - i : i;
                    const e = fn(getValue(data, index), index, data);
                    if (e != null) {
                        array[index] = e;
                    }
                }
    ...
    Array.prototype.map = function (fn, order) {
        return map_factory(this)((arr, i) => arr[i])(fn, order);
    };
    Array.prototype.filter = function (fn) {
        const old_array = this as any[];
        const new_array = [];
    ...
    Object.prototype.assign = function (config) {
        config.each((v, k) => { this[k] = v; });
        return this;
    };
    PropertyGroup.prototype.map = function (fn, order) {
        return map_factory(this, arr => arr.numProperties)((arr, i) => arr(i + 1))(fn, order);
    };
