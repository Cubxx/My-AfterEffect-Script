(function (that) {
    ...
    class KeyframeData {
        ...
        /**判断属性值类型是否为空间值 */
        static isSpatial(property: Property) {
            return !![PropertyValueType.TwoD_SPATIAL, PropertyValueType.ThreeD_SPATIAL].filter((e): e is number => e === property.propertyValueType).length;
        }
        /**入点插值类型 */
        InInterpolationType = KeyframeInterpolationType.LINEAR
        OutInterpolationType = KeyframeInterpolationType.LINEAR
        /**入点缓动 */
        InTemporalEase = [new KeyframeEase(0, 0.1)] as [KeyframeEase] /* | [KeyframeEase, KeyframeEase] | [KeyframeEase, KeyframeEase, KeyframeEase] */
        OutTemporalEase = [new KeyframeEase(0, 0.1)] as [KeyframeEase] /* | [KeyframeEase, KeyframeEase] | [KeyframeEase, KeyframeEase, KeyframeEase] */
        /**时间自动贝塞尔 */
    ...
    const c = {
        _any2obj: {
            ...
            'MarkerValue'(v: MarkerValue) { return v; },
            'Shape'(v: Shape) { return v; },
            ...
            'KeyframeEase'(v: KeyframeEase) { return v; },
        ...
        _obj2any: {
            ...
            'MarkerValue'(v: Partial<MarkerValue>) { return new MarkerValue(''); },
            'Shape'(v: Partial<Shape>) {
                const obj = new Shape();
                v.each((value, key: string) => { obj[key] = value });
                return obj;
            },
            'TextDocument'(v: Partial<TextDocument>) { return v; },
        ...
        type(value: unknown) {
            ...
            if (value instanceof Array) {
                ...
                else if (value.filter((e): e is object => typeof e === 'object').length === value.length) {
                    return 'object[]';
                }
            }
        },
    ...
    const l = {
        ...
        recur_factory: ((mode: 'get' | 'set', {
            ...
            (mode: 'get', { GroupFn, PropertyFn, }: {
                GroupFn?: (p: PropertyGroup | MaskPropertyGroup, d: {}) => boolean;
                PropertyFn?: (p: Property, d: {}) => boolean;
            }): (p: _PropertyClasses, d: {}) => void | {};
            (mode: 'set', { GroupFn, PropertyFn, }: {
                GroupFn?: (p: PropertyGroup | MaskPropertyGroup, d: {}) => boolean;
                PropertyFn?: (p: Property, d: {}) => boolean;
            }): (p: _PropertyClasses, d: {}) => _PropertyClasses;
        },
        /**
         * 递归获取 PropertyGroup 下的所有 _PropertyClasses  
         * 并对不同的 _PropertyClasses 执行不同函数
         */
        get_group(group: PropertyGroup | MaskPropertyGroup, data: {}, recur: (p: _PropertyClasses, d: {}) => void | {}) {
            data['@matchName'] = group.matchName;
            PropertyGroup.prototype.map.call(group, (property: _PropertyClasses) => {
        ...
        set_group(group: PropertyGroup | MaskPropertyGroup, data: {}, recur: (p: _PropertyClasses, d: {}) => _PropertyClasses) {
            data.each((sub_data, key) => {
                ...
                const matchName = sub_data['@matchName'], name = key;
                if (matchName) {
        ...
        get_layer(layer: Layer, propertyNames = ['']) {
            const data = {};
            [
                // Layer
                'name',
                ...
                'stretch',
                'outPoint',
            ].concat(propertyNames).map(key => {
                data[key] = layer[key];
            }, true);
            function defaultFn(p: _PropertyClasses, d: {}) {
                if (p.isModified) {
            ...
            l.get_group(layer, data, l.recur_factory('get', {
                GroupFn(p, d) {
                    switch (p.parentProperty.matchName) {
                        ...
                        case 'ADBE Effect Parade': return true; // 所有效果都要导出
                ...
                PropertyFn(p, d) {
                    switch (p.parentProperty.matchName) {
                        case 'ADBE Text Animator Properties': { //文本动画设计器属性
    ...
    const h = {
        ...
        export_layer(layer: Layer) {
            ...
            // 判断图层类型
            const type: string | void = matchTypes[layer.matchName];
            switch (type) {
                case 'Text': {
                    propertyNames.push('threeDPerChar');
                }
                case 'Shape': {
                    (layer as TextLayer | ShapeLayer).canSetCollapseTransformation && propertyNames.push('collapseTransformation');
                    (layer as TextLayer | ShapeLayer).canSetTimeRemapEnabled && propertyNames.push('timeRemapEnabled');
                    break;
                }
                case 'Camera': ;
                case 'Light': ;
        ...
        config_read() { config.user = JSON.parse(f.read(p.user_config())); },
        config_write() { f.write(p.user_config(), JSON.stringify(config.user)); },
    };
    const p = {
        layers: () => [config.root_path, config.name, 'layers'].join('\\'),
        download: () => [config.root_path, config.name, config.name + '.jsx'].join('\\'),
        local: () => [Folder.appPackage.fsName, 'Scripts', 'ScriptUI Panels', config.name + '.jsx'].join('\\'),
    ...
    const a = {
        ...
        UI() {
            ...
            btn(win, File.decode(config.icons.setting)).onClick = function () {
                h.config_read();
                const { user } = config;
                const win = u.dialog(void 0, config.name + ' ' + config.version).assign({
                    margins: 10,
                });
